...
data <- data %>%
  ...
  FindClusters(resolution = seq(from = 0.1, to = 2, by = 0.1))
```

```{r all resolutions, fig.height=10, fig.width=10}
DimPlot(data, group.by = paste0("RNA_snn_res.", seq(from = 0.1, to = 2, by = 0.1)), label = TRUE) & NoLegend()
```
...
```{r eval = FALSE}
Idents(data) <- "seurat_clusters"
new.cluster.ids <- c("Myeloid cells", "T cells", "Myeloid cells", "Myeloid cells", "Macrophages")
names(new.cluster.ids) <- c(0:4)
...
data$sample <- sample
data$annot_2 <- paste(data$annot, data$RNA_snn_res.2)
```
...
<!-- ```{r fig.width=10} -->
<!-- genes <- c("TACC3", "FGFR3", "GATA3", "EGFR", "EPCAM", "BRAF") -->
<!-- FeaturePlot( -->
<!--   data, -->
<!--   features = genes, -->
<!--   cols = c("#ADD8E633", "#E46726"), -->

...
saveRDS(data, glue("{out_dir}/{proj_name}_{library}_integrated.rds"))
```
...
```{r eval = FALSE}
Idents(data) <- "seurat_clusters"
new.cluster.ids <- c("Macrophages", "Macrophages", "DC", "T cells", "Monocytes", "bad quality", "T cells", "Macrophages", "T cells", "bad quality", "Proliferative", "DC", "T cells", "Tumor cells", "Fibroblasts", "Tumor cells")
names(new.cluster.ids) <- c(0:15)
...
```{r remove clusters, eval = FALSE}
Idents(data) <- "annot_2"
data <- CellSelector(plot = DimPlot(data), object = data, ident = "T cells 10")
data$annot_2 <- as.character(Idents(data))
Idents(data) <- as.character(Idents(data))
```
...
# save file
saveRDS(markers, glue("{out_dir}/{proj_name}_{library}_general_cluster_markers_res1.rds"))
# write to excel file
openxlsx::write.xlsx(split(markers, markers$cluster), 
                     file = glue("{out_dir}/{proj_name}_{library}_general_cluster_markers_res1.xlsx"),
                     overwrite = TRUE)
```
...
data$Index <- paste0(str_replace(rownames(data@meta.data), pattern = glue("{proj_name}_{library}_"), replacement = ""), "-1")

data$Cell_type <- str_replace_all(data$annot_2, pattern = " ", replacement = "_")
data@meta.data[,c("Index", "Cell_type")] %>% 
...
data@meta.data[, c("sample", "Frequency", "cloneType", "CTaa", "annot")] %>% 
  filter(annot %in% c("T cells", "B cells", "Plasma B cells")) %>% 
  unique() %>% 

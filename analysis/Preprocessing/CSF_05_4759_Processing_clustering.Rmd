...
```{r eval = FALSE}
Idents(data) <- "seurat_clusters"
new.cluster.ids <- c("T cells", "T cells", "T cells", "T cells", "B cells", "Myeloid", "pDC", "T cells", "T cells", "cDC", "DC")
...
data <- FindSubCluster(data, cluster = 6, graph.name = "RNA_snn", resolution = 1)

data$annot_2[data$annot == "Myeloid"] <- paste("Myeloid", data$sub.cluster[data$annot == "Myeloid"])
```
...
FeaturePlot(
  subset(data, annot == "B cells"),
  features = c("MS4A1", "CD79A", "CD19", "CD200"),
  order = TRUE,
  pt.size = 1.5,

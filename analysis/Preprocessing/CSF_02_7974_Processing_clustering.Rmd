...
```{r eval = FALSE}
Idents(data) <- "seurat_clusters"
new.cluster.ids <- c("T cells", "B cells", "T cells", "Myeloid", "T cells", "B cells", "T cells - NK", "Tumor", "Proliferative", "Plasma B cells", "DC1", "pDC", "Fibroblasts")
names(new.cluster.ids) <- c(0:12)
...
```{r eval = FALSE}
data <- subset(data, annot_2 %in% c("T cells 21", "T cells 11"), invert = TRUE)
# save annotated object
saveRDS(data, glue("{out_dir}/{proj_name}_{library}_annotated.rds"))
```
...
data@meta.data[, c("sample", "Frequency", "cloneType", "CTaa", "annot")] %>% 
  filter(annot %in% c("T cells", "T cells - NK", "B cells", "Plasma B cells")) %>% 
  unique() %>% 
...
t1 <- table(!is.na(subset(data, annot %in% c("T cells", "T cells - NK", "B cells"))$cloneType)) %>% as.data.frame()

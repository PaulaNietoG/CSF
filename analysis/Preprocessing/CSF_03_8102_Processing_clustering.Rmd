...
```{r eval = FALSE}
Idents(data) <- "seurat_clusters"
new.cluster.ids <- c("T cells", "Myeloid", "B cells", "T cells", "T cells", "Plasma cells", "T cells", "Proliferative")
names(new.cluster.ids) <- c(0:7)
...
data@meta.data[, c("sample", "Frequency", "cloneType", "CTaa", "annot")] %>% 
  filter(annot %in% c("T cells", "B cells", "Plasma cells")) %>% 
  unique() %>% 
...
FeaturePlot(
  subset(data, annot == "B cells"),
  features = c("MS4A1", "BCL2", "BCL6", "MME", "MYC"),
  order = TRUE,
  pt.size = 1.5,

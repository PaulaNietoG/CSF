...
```{r}
DimPlot(data, group.by = "type", cols = c("grey", "#d6604d"), pt.size = 2) + labs(caption = "5 tumor cells (P05)")
```
...
FeaturePlot(
  data,
  features = c("GFAP", "PTPRC"),
  order = TRUE,
  pt.size = 1.5,
  cols = c("#ADD8E633", "#d6604d"),
)
```
...
```{r}
data <- PercentageFeatureSet(data, "^RP", col.name = "percent_ribo")
```
